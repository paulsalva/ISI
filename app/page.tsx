...
export default function Home() {
  return (
    <>
      {/* ── Hero ── */}
      <section
        className="grid-texture hero-section"
        style={{
          position: 'relative',
      ...
      >
        <div className="stats-grid" style={{ maxWidth: '1280px', margin: '0 auto' }}>
          {stats.map((s, i) => (
            <div key={i} className={`stat-item ${i < stats.length - 1 ? 'stat-border' : ''}`}>
              <div className="display" style={{ fontSize: '36px', lineHeight: 1 }}>
                {s.num}
              </div>
      ...
      {/* ── Services ── */}
      <section className="section-pad" style={{ maxWidth: '1280px', margin: '0 auto', paddingBottom: 0 }}>
        <p className="section-label">Our Solutions</p>
        <h2 className="display" style={{ fontSize: 'clamp(30px, 4vw, 42px)', marginBottom: '12px' }}>
      ...
      >
        {services.map((svc) => (
          <div key={svc.num} className="service-card">
            ...
            <h3 className="display" style={{ fontSize: '22px', marginBottom: '12px', lineHeight: 1.2 }}>
              {svc.title}
            </h3>
            <p style={{ fontSize: '13px', color: 'var(--color-fg-muted)', lineHeight: '1.75', marginBottom: '20px' }}>
              {svc.body}
            </p>
            <Link href={svc.href} style={{ fontSize: '11px', letterSpacing: '0.1em', textTransform: 'uppercase', color: 'var(--color-accent)', textDecoration: 'none' }}>
              Learn more →
            </Link>
          </div>
        ))}
      ...
      {/* ── Industries ── */}
      <section style={{ background: 'var(--color-bg-surface)', padding: '60px 40px' }}>
        <div style={{ maxWidth: '1280px', margin: '0 auto' }}>
          <p style={{ fontSize: '11px', letterSpacing: '0.2em', textTransform: 'uppercase', color: 'var(--color-fg-dim)', marginBottom: '32px' }}>
            Industries Served
          </p>
          <div
          ...
          >
            {industries.map((ind) => (
              <Link href="/industries" key={ind} style={{ textDecoration: 'none' }} className="industry-card">
                <div style={{ width: '4px', height: '4px', borderRadius: '50%', background: 'var(--color-accent)', marginBottom: '14px' }} />
                <div style={{ fontSize: '13px', color: 'var(--color-fg-muted)', lineHeight: '1.4' }}>{ind}</div>
              </Link>
            ))}
          </div>
        </div>
      ...
      {/* ── Clients ── */}
      <section style={{ padding: '56px 40px', borderTop: '1px solid var(--color-border)' }}>
        <div style={{ maxWidth: '1280px', margin: '0 auto' }}>
          <p style={{ fontSize: '11px', letterSpacing: '0.2em', textTransform: 'uppercase', color: 'var(--color-fg-faint)', textAlign: 'center', marginBottom: '32px' }}>
            Trusted By
          </p>
      ...
      {/* ── CTA ── */}
      <section style={{ background: 'var(--color-accent)', padding: '72px 40px' }}>
        <div className="cta-row" style={{ maxWidth: '1280px', margin: '0 auto' }}>
          <h2 style={{ fontFamily: 'var(--font-cormorant), Georgia, serif', fontSize: 'clamp(28px, 4vw, 40px)', fontWeight: 600, color: '#fff', lineHeight: '1.15', maxWidth: '480px' }}>
            Ready to engineer your{' '}
            <span style={{ fontStyle: 'italic', opacity: 0.75 }}>next-generation</span>{' '}
            network?
          </h2>
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: '12px' }}>
            <span style={{ fontSize: '11px', letterSpacing: '0.12em', textTransform: 'uppercase', color: 'rgba(255,255,255,0.6)' }}>
              Speak with an Engineer
            </span>
            <a href="tel:18005054742" style={{ fontSize: '26px', fontWeight: 500, color: '#fff', textDecoration: 'none', letterSpacing: '0.02em' }}>
              800-505-ISIC
            </a>
